	...
	data.features.forEach((feature,i) => {		
		...
		if (thisCounty==county) {
			...
			$('#fulltable tbody').append('<tr id="'+id+'" class="'+feature.geometry.certainty+'">'
				+'<td>'+glyphs+'</td>'
				+(feature.geometry.certainty=='certain'?'<td>'+feature.properties.title+kvdb+'</td>':'<td><a title="Click to open in map view, where you can submit suggestions." href="../#/10/'+coordinates[0]+'/'+coordinates[1]+'/mode=points+selected=IV%3A'+id+'">'+feature.properties.title+'</a>'+kvdb+'</td>')
